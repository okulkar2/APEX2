...
public class Pipeline {
	...
	public void write(){
		...
		if(stages.get("WB") != null){
			...
			if(pipelinestages.get("WB").getRegister() != null){
				...
				pipeMem.setPipelineRegister(pipelinestages.get("WB").getRegister(), pipelinestages.get("WB").getValue());
				pipelinestages.put("WB1", pipeMem);
				if(pipeResult.getRegister() != null && pipeResult.getRegister().equalsIgnoreCase(currentInst.getDestination())){
					Register result = registerFile.get(pipeResult.getRegister());
				...
				PipelineRegister pipeMem = pipelinestages.get("WB1");
				pipeMem.setPipelineRegister(null, pipelinestages.get("WB1").getValue());
				pipelinestages.put("WB1", pipeMem);
			}
			stages.put("WB", null);
		}
	...
	public void writeBack() {
		...
		if (stages.get("MEM") != null) {
			...
			if(pipelinestages.get("MEM").getRegister() != null){
				pipeResult.setPipelineRegister(pipelinestages.get("MEM").getRegister(), pipelinestages.get("MEM").getValue());
				//System.out.println("WB Register:"+pipeResult.getRegister()+" Value:"+pipeResult.getValue());
				pipelinestages.put("WB", pipeResult);
			}
			else{
				pipeResult.setPipelineRegister(null, pipelinestages.get("MEM").getValue());
	...
	public void memStage() {
		...
		if (stages.get("IntALU2") != null) {
			...
			if(currentInst.getOperand().equals("LOAD") || currentInst.getOperand().equalsIgnoreCase("STORE")){
				if(currentInst.getOperand().equalsIgnoreCase("STORE")){
					if(Flag.isForwardStoreFlagSet()==true && pipelinestages.get("WB1").getRegister()!=null && pipelinestages.get("WB1").getRegister().equalsIgnoreCase(currentInst.getSource1())){
						currentInst.setSrc1Value(pipelinestages.get("WB1").getValue());
						Flag.setForwardStoreFlag(false);
					}
				}
				int result = MEMStage.executeIntructionMEM(currentInst,pipelinestages.get("EX2").getValue());
				if(currentInst.getOperand().equals("LOAD")){
					pipeResult.setPipelineRegister(currentInst.getDestination(), result);
				}
				else{
				...
				if(pipelinestages.get("EX2").getRegister()!=null){
					pipeResult.setPipelineRegister(pipelinestages.get("EX2").getRegister(), pipelinestages.get("EX2").getValue());
					//System.out.println("MEM Register:"+pipeResult.getRegister()+" Value:"+pipeResult.getValue());
				}
				else{
					pipeResult.setPipelineRegister(null, pipelinestages.get("EX2").getValue());
	...
	private void execute2(){
		...
		if (stages.get("BranchALU") != null) {
			...
			if(currentInst.getDestination()==null){
				PipelineRegister pipeResult = pipelinestages.get("EX2");
				pipeResult.setPipelineRegister(null, pipelinestages.get("Branch").getValue());
				//System.out.println("EX2 Register:null Value:"+pipeResult.getValue());
				pipelinestages.put("EX2", pipeResult);
			}
	...
	private void execute1(){
		...
		if (stages.get("D/RF") != null) {
				...
				if(Flag.isStallFlagSet() == false){
					...
					if(currentInst.getSource2() != null){
						if(Flag.isForwardEXFlagSet()==true && pipelinestages.get("MEM").getRegister()!=null && pipelinestages.get("MEM").getRegister().equalsIgnoreCase(currentInst.getSource2())){
							...
							if(Flag.isForwardMEMFlagSet()==true && pipelinestages.get("WB").getRegister()!=null && pipelinestages.get("WB").getRegister().equalsIgnoreCase(currentInst.getSource2())){
								currentInst.setSrc2Value(pipelinestages.get("WB").getValue());
								Flag.setForwardMEMFlag(false);
							}
							else{
								currentInst.setSrc2Value(registerFile.get(currentInst.getSource2()).getValue());
							}
						}
					}
				
					if(currentInst.getDestination()!=null){
						Register result = registerFile.get(currentInst.getDestination());
			...
			else if(currentInst.getFunction_unit().equalsIgnoreCase("BranchALU")){
	...
	public void fetch() {
		...
		if(stages.get("F") == null && Flag.isBranchFlagSet()==false){
			if((fetchInst = Cache.getInstruction()) != null){
				...
				//System.out.println("Fetched Instruction: " + programCounter + " " + fetchInst);
